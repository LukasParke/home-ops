...
3. An **inline NFS volume** for `/data` (`${DOWNLOADS_NFS_PATH}`, same `${MEDIA_NFS_SERVER}`) so the `*arr` pods can see the download client's completed files. The download client (qBittorrent on Unraid) mounts the same share at `/data`, so it reports `/data/...` paths the `*arr` containers already see — **no Remote Path Mapping is needed** as long as the download client's container path is `/data`. Ensure the Unraid share at `${DOWNLOADS_NFS_PATH}` is **NFS-exported** before applying — the cluster mount will fail with `FailedMount` otherwise.
